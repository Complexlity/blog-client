...
function extractTitleAndContent(content: string) {
  ...
  if (lineIsHeaderOne(firstLine)) {
    ...
    if (heading1.length < 50) {
      title = capitalize(heading1);
      //Change any others
      newContent = others.replace(/(^# .+\n?)/, "## ");
      newContent = `# ${title}\n${newContent}`;
    } else {
      let longTitle = heading1.split(/\s+/);
      title = capitalize(longTitle.slice(0, 2).join(" "));
      newContent = `# ${title}\n##${longTitle}\n${others.replace(
        /^# /,
        "## "
      )}`;
    ...
    if (firstLine.length > 50) {
      ...
      title = capitalize(firstLine.replace(/^#+\s*/, ""));
      newContent = others.replace(/(^# .+\n?)/, "## ");
      newContent = `# ${title}\n${newContent}`;
    }
  }
...
function replaceAllH1WithH2(htmlString: string): string {
  ...
  const handleFileChange = (event: any) => {
    ...
    if (
      ...
      reader.onload = async (e) => {
        //@ts-expect-error
        const { title, content } = extractTitleAndContent(e.target.result);
